...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        # If array is not compact, materialize a compact copy first
        ...
        # For compact arrays, reshape can be done by reinterpreting strides
        return NDArray.make(
            new_shape,
            strides=NDArray.compact_strides(new_shape),
            device=self.device,
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        # If old has more dims, squeeze leading singleton dims
        if len(old_shape) > len(new_shape):
            drop = len(old_shape) - len(new_shape)
            # all dropped dims must be 1
            assert all(d == 1 for d in old_shape[:drop]), (
                "Cannot reduce non-singleton dims in broadcast: %s -> %s" % (old_shape, new_shape)
            )
            old_shape = old_shape[drop:]
            old_strides = old_strides[drop:]

        # If old has fewer dims, pad leading singleton dims
        if len(old_shape) < len(new_shape):
            pad = len(new_shape) - len(old_shape)
            old_shape = (1,) * pad + old_shape
            # for padded singleton dims, stride is irrelevant (use 0)
            old_strides = (0,) * pad + old_strides

        new_strides: list[int] = []
        for osz, nsz, st in zip(old_shape, new_shape, old_strides):
            if osz == nsz:
                new_strides.append(st)
            else:
                # only allow broadcasting when original size is 1
                assert osz == 1, "Incompatible broadcast: %s -> %s" % (old_shape, new_shape)
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for d, (sl, stride_d) in enumerate(zip(slices, self.strides)):
            start, stop, step = sl.start, sl.stop, sl.step
            new_offset += start * stride_d
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        new_strides = tuple(
            -self.strides[i] if i in axes else self.strides[i]
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        out = self.device.full(out_shape, 0.0)
        slices = tuple(
...
def where(cond, x, y):
    """Elementwise selection on NDArray backend: returns x where cond!=0 else y.
    cond: NDArray (0/1 floats)
    x, y: NDArray or python floats; will be broadcast to cond.shape by construction here.
    """
    ...
    def to_nd(val):
        if isinstance(val, NDArray):
            return val if val.shape == cond.shape else val.broadcast_to(cond.shape)
        else:
            return full(cond.shape, float(val), device=cond.device)

    xa = to_nd(x)
    ya = to_nd(y)
    return cond * xa + (1 - cond) * ya
